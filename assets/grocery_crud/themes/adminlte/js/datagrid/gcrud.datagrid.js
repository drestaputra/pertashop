...
(function ($) {
    ...
    var Datagrid = function (gcrud_container) {
        ...
        this.facadeInitListeners = function () {
            ...
            this.listenerPrintCustomButton();
    ...
    Datagrid.prototype.clearFilteringCache = function () {
        var $searchAllInput = $('.search-button > input.search-input');

        $('.page-number-hidden').val('1');
        $('.page-number-input').val('1');
        $('.grocery-crud-table tr:first').find('.active')
            .removeClass('active ordering-desc ordering-asc');
        $('.grocery-crud-table tr:first').find('.fa-chevron-up,.fa-chevron-down').remove();
        $('td.active').removeClass('active');
        $('thead .value-not-empty').removeClass('value-not-empty').val('');
    ...
    Datagrid.prototype.checkCachedData = function () {
        var perPageValue = $('.' + Datagrid.CLASS_PER_PAGE).val();
        ...
        if (perPageValue !== cachedDatra.per_page) {
            $('.' + Datagrid.CLASS_PER_PAGE).val(cachedDatra.per_page);
            forceRefresh = true;
        }

        if (cachedDatra.page !== "1") {
            $('.page-number-hidden').val(cachedDatra.page);
            $('.page-number-input').val(cachedDatra.page);
            forceRefresh = true;
        }

        if (cachedDatra.order_by[0] !== null) {
            var $column_to_order = $('.grocery-crud-table tr:first')
    ...
    Datagrid.prototype.datagridInit = function () {
        var success_message_container = $('.success-message');
    ...
    Datagrid.prototype.appendSearchClearButtons = function () {
        ...
        $('.grocery-crud-table input.searchable-input').each(function () {
    ...
    Datagrid.prototype.listenerPerPage = function () {
        ...
        $('.' + Datagrid.CLASS_PER_PAGE).change(function () {
            $('.page-number-hidden').val('1');
            datagrid_object.SearchAndOrderingTrigger();
        });
    };
        ...
        $('.gc-export').click(function () {
            ...
            if ($('.search-input').val() !== '') {
                form_inputs = form_inputs + '<input type="hidden" name="search_field" value="" />' +
                    '<input type="hidden" name="search_text" value="' + $('.search-input').val() + '" />';
            } else {
                $('.grocery-crud-table thead input').each(function () {
    ...
    Datagrid.prototype.listenerPrintCustomButton = function () {
        $('.print-selected-button').click(function () {
            
                event.preventDefault();
                var print_selected = [];
                var urlPrint = $(".print-selected-button").attr('data-url');
                var data_to_send;
                $('.select-row:checked').each(function () {
                    print_selected.push($(this).data('id'));
                });
    ...
    Datagrid.prototype.pagingCalculations = function () {
        var page_number_value   = parseInt($('.page-number-hidden').val(), 10),
        ...
        if (page_number_value === 1) {
            $('.paging-first').addClass(Datagrid.CLASS_DISABLED);
            $('.paging-previous').addClass(Datagrid.CLASS_DISABLED);
        } else {
            $('.paging-first').removeClass(Datagrid.CLASS_DISABLED);
            $('.paging-previous').removeClass(Datagrid.CLASS_DISABLED);
        }

        if (page_number_value + 1 > max_paging) {
            $('.paging-last').addClass(Datagrid.CLASS_DISABLED);
            $('.paging-next').addClass(Datagrid.CLASS_DISABLED);
        } else {
            $('.paging-last').removeClass(Datagrid.CLASS_DISABLED);
            $('.paging-next').removeClass(Datagrid.CLASS_DISABLED);
        }

        $('.page-number-input').val(page_number_value);

        if (page_number_value > max_paging) {
            $('.page-number-hidden').val(max_paging);
            $('.page-number-input').val(max_paging);
        }
    ...
    Datagrid.prototype.SearchAndOrderingTrigger = function () {
        var order_by = $('.column-with-ordering.active:first').data('order-by'),
            ...
            search_all_input = $('.search-button > input.search-input'),
            per_page_value = $('.' + Datagrid.CLASS_PER_PAGE).val(),
            data_to_send;
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('.select-all-none').prop('checked', false);

                $('.grocery-crud-table tbody').html(result.tbody_html);
                ...
                if (result.current_total_results < parseInt($('.full-total').html(), 10)) {
                    $('.full-total-container').removeClass('hidden');
                } else {
                    $('.full-total-container').addClass('hidden');
                }
    ...
    Datagrid.prototype.listenerSearchButton = function () {
        ...
        $('.search-button>input.search-input').change(function () {
            $('.search-button .clear-all-search').remove();

            if ($(this).val() !== '') {
                $(this).after('<i class="fa fa-times clear-all-search"></i>');

                $('.search-button .clear-all-search').click(function () {
                    $('.search-button>input.search-input').val('').trigger('change');
                });
            }
    ...
    Datagrid.prototype.getMaxPaging = function () {
        var total_results = parseInt($('.current-total-results').html(), 10),
            per_page = parseInt($('.' + Datagrid.CLASS_PER_PAGE).val(), 10),
    ...
    Datagrid.prototype.listenerPagingButtons = function () {
        var datagrid_object = this,
            ...
            page_number_input_hidden = $('.page-number-hidden');

        $(selectors_for_all_buttons).click(function (event) {
            event.preventDefault();
        ...
        $('.page-number-input').change(function () {
    ...
    Datagrid.prototype.hideShowDeleteButton = function () {
        ...
        if ($('.select-row:checked').length > 0) {
            ...
            $('.print-selected-button').removeClass('hidden');
        } else {
            $('.delete-selected-button').addClass('hidden');
            $('.export-selected-button').addClass('hidden');
            $('.print-selected-button').addClass('hidden');
        }
    };

    Datagrid.prototype.calculationsBeforeDelete = function (total_deleted_records) {
        var datagrid_object = this,
            my_current_total = parseInt($('.current-total-results').html(), 10),
            my_current_page_number = parseInt($('.page-number-input').val(), 10),
            my_current_per_page = parseInt($('.' + Datagrid.CLASS_PER_PAGE).val(), 10);
        ...
        if (my_current_page_number === datagrid_object.getMaxPaging() &&
            ...
            $('input[name="page_number"]').val(datagrid_object.getMaxPaging() - 1);
        }

        $('.full-total').html(
            (parseInt($('.full-total').html(), 10) - total_deleted_records)
        );
    };
    ...
    Datagrid.prototype.listenerDeleteMultiple = function () {
        ...
        $('.delete-selected-button').click(function () {
            ...
            $('.delete-multiple-confirmation').on('hidden.bs.modal', function () {
                $('.delete-multiple-confirmation-button').unbind('click');
            });

            $('.delete-multiple-confirmation-button').click(function (event) {
                event.preventDefault();

                var my_modal = $('.delete-multiple-confirmation:first'),
                    my_current_total = parseInt($('.current-total-results').html(), 10),
                    my_current_page_number = parseInt($('.page-number-input').val(), 10),
                    my_current_per_page = parseInt($('.' + Datagrid.CLASS_PER_PAGE).val(), 10),
                    delete_selected = [],
                ...
                $.ajax({
                    ...
                    method: 'post',
                    dataType: 'json',
                    url: $('.delete-multiple-confirmation-button').data('target'),
    ...
    Datagrid.prototype.listenerLoadMoreButton = function () {
        ...
        $('table.grocery-crud-table>tbody .dropdown').on('hide.bs.dropdown', function () {
