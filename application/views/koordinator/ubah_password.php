...
$('form#change_password').submit(function(event) {
    ...
    if (confirm("Yakin mengubah password?")) {
      $.ajax({
        url: '<?php echo base_url().'koordinator/change_password/'.$id_koordinator ?>',
        type: 'POST',
        dataType: 'JSON',
        data: formData,
        success :function (response){
          if (response.status == 200) {
            window.location = '<?php echo base_url("koordinator?status=200&msg=").base64_encode("Berhasil mengubah koordinator"); ?>';
          }else{
