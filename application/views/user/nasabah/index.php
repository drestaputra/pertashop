...
<html class="fixed">
	<head>
		...
		<!-- Basic -->
		<meta charset="UTF-8">
		<title>Nasabah | <?php echo function_lib::get_config_value('website_name'); ?></title>
	...
	<body>
		...
		<script type="text/javascript">		
			$(document).ready(function() {
				...
				<?php endif ?>
				<?php if (isset($state) AND $state=="list"): ?>
					$('.table-responsive [name=status]').attr('name','nasabah.status');
					$('.table-responsive [name=username]').attr('name','nasabah.username');
					$('.table-responsive [name=nama]').attr('name','nasabah.nama');
					$('.table-responsive [name=email]').attr('name','nasabah.email');
					$('.table-responsive [name=no_hp]').attr('name','nasabah.no_hp');
					$('.table-responsive [name=warga_negara]').attr('name','nasabah.warga_negara');
				<?php endif ?>
				$('#field-provinsi').change(function() {
					var selectedValue = $('#field-provinsi').val();
					// alert('selectedValue'+selectedValue);
					//alert('post:'+'ajax_extension/kabupaten/id_provinsi/'+encodeURI(selectedValue.replace(/\//g,'_agsl_')));
			...
			$('#field-kecamatan').change(function() {
					var selectedValue = $('#field-kecamatan').val();		
					console.log(selectedValue)			;
					$.post('ajax_extension/kelurahan/id_kecamatan/'+encodeURI(selectedValue.replace(/\//g,'_agsl_')), {}, function(data) {
					var $el = $('#field-kelurahan');
						  var newOptions = data;
						  $el.empty(); // remove old options
						  $el.append($('<option></option>').attr('value', '').text(''));
						  $.each(newOptions, function(key, value) {
