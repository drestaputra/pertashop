...
<head>
  ...
  <!-- Basic -->
  <meta charset="UTF-8">
  <title>Edit Admin | <?php echo function_lib::get_config_value('website_name'); ?></title>
                        ...
                        <fieldset>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" name="username" value="<?php echo (trim($admin['username'])!="")?$admin['username']:""; ?>" class="form-control" placeholder="Username .." required/>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" name="email" value="<?php echo (trim($admin['email'])!="")?$admin['email']:""; ?>" class="form-control" placeholder="Email .." required/>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select class="form-control" name="status">
                                       <option value="aktif" <?php echo ($this->input->post('status')!="")?(($this->input->post('status')=="aktif")?'selected':''):(($admin['status']=='aktif')?'selected':''); ?>>Aktif</option>
                                       <option value="pending" <?php echo ($this->input->post('status')!="")?(($this->input->post('status')=="pending")?'selected':''):(($admin['status']=='pending')?'selected':''); ?>>Pending</option>
                                       <option value="blokir" <?php echo ($this->input->post('status')!="")?(($this->input->post('status')=="blokir")?'selected':''):(($admin['status']=='blokir')?'selected':''); ?>>Blokir</option>                                                   
                                   </select>
                               </div>                                            
                           </div>                                  
                       </fieldset> 
                       <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-9 col-md-offset-3">
                                ...
                                <a href="<?php echo base_url('user/admin'); ?>" class="btn btn-default">Cancel</a>
                            </div>
                        </div>
                    </div>                                      
...
$('form#form-profil').submit(function(event) {  
    ...
    if (confirm("Yakin simpan?")) {
      $.ajax({
        url: '<?php echo base_url().'user/admin/edit/'.$id_admin ?>',
        type: 'POST',
        dataType: 'JSON',
        data: formData,
