...
<head>
  ...
  <!-- Basic -->
  <meta charset="UTF-8">
  <title>Ubah Password User | <?php echo function_lib::get_config_value('website_name'); ?></title>
  <meta name="keywords" content="Dashboard User - <?php echo function_lib::get_config_value('website_name'); ?>" />
...
<body>
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">Ubah Password User</h3>
    </div>
                    ...
                    <div class="panel-footer">
                        <div class="row">
                            <div class="col-md-9 col-md-offset-3">
                                ...
                                <a href="<?php echo base_url('user/user'); ?>" class="btn btn-default">Cancel</a>
                            </div>
                        </div>
                    </div>
                </form>                                     
...
<?php $this->load->view('user/user/right_bar'); ?>
</section>
<!-- Vendor -->
...
$('form#change_password').submit(function(event) {
    ...
    if (confirm("Yakin mengubah password?")) {
      $.ajax({
        url: '<?php echo base_url().'user/user/change_password/'.$id_user ?>',
        type: 'POST',
        dataType: 'JSON',
        data: formData,
        success :function (response){
          if (response.status == 200) {
            window.location = '<?php echo base_url("user/user?status=200&msg=").base64_encode("Berhasil mengubah user"); ?>';
          }else{
