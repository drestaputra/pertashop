...
<head>
  ...
  <!-- Basic -->
  <meta charset="UTF-8">
  <title>Tambah Admin | <?php echo function_lib::get_config_value('website_name'); ?></title>
        ...
        <div class="panel-footer">
            <div class="row">
                <div class="col-md-9 col-md-offset-3">
                    ...
                    <a href="<?php echo base_url('user/admin'); ?>" class="btn btn-default" onclick="return confirm('Batal menambah?')" >Batal</a>
                </div>
            </div>
        </div>                                      
...
$('form').submit(function(event) {
    ...
    if (confirm("Yakin simpan?")) {
      $.ajax({
        url: '<?php echo base_url().'user/admin/tambah' ?>',
        type: 'POST',
        dataType: 'JSON',
        data: formData,
        success :function (response){
          if (response.status == 200) {
            window.location = '<?php echo base_url("user/admin?status=200&msg=").base64_encode("Berhasil menambah admin"); ?>';
          }else{
