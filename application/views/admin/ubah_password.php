...
<head>
  ...
  <!-- Basic -->
  <meta charset="UTF-8">
  <title>Ubah Password Admin | <?php echo function_lib::get_config_value('website_name'); ?></title>
...
<body>
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title">Ubah Password Admin</h3>
    </div>
...
$('form#change_password').submit(function(event) {
    ...
    if (confirm("Yakin mengubah password?")) {
      $.ajax({
        url: '<?php echo base_url().'admin/change_password/'.$id_admin ?>',
        type: 'POST',
        dataType: 'JSON',
        data: formData,
        success :function (response){
          if (response.status == 200) {
            window.location = '<?php echo base_url("admin?status=200&msg=").base64_encode("Berhasil mengubah admin"); ?>';
          }else{
